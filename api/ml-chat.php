...
$mlAvailable = false;
if (file_exists('../vendor/autoload.php') && file_exists('../includes/ml-emotion-detector.php')) {
    try {
        require_once '../vendor/autoload.php';
        require_once '../includes/ml-emotion-detector.php';
        $mlAvailable = true;
    } catch (Exception $e) {
        error_log('ML not available: ' . $e->getMessage());
        $mlAvailable = false;
    }
}
...
try {
    ...
    echo json_encode([
        ...
        'allProbabilities' => $usingML ? $predictionResult['all_probabilities'] : [$detectedEmotion => 0.75],
        'recommendedSongs' => $recommendedSongs,
        'mlPowered' => $usingML,
        'model' => $usingML ? 'Rubix ML - K-Nearest Neighbors' : 'Keyword-based (ML not installed)'
    ]);
